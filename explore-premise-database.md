...
year_list=['2020','2050']
SSP_list=['SSP1','SSP2']
IAM_scenario_list=['Base','RCP26','RCP45','Npi','RCP19']
...
#Example
selected_db_list=[db for db in premise_db_list if db.IAM_scenario=='RCP45']
...
dict_color={
    ...
    "heat and power co-generation, wood chips, 6667 kW":['chartreuse','biomass, biogas and waste'],
    "heat and power co-generation, biogas, gas engine":['chartreuse','biomass, biogas and waste'],
    "treatment of municipal solid waste, incineration":['chartreuse','biomass, biogas and waste'],

    "electricity production, deep geothermal":['blue','wave & geothermal'],
    "electricity production, wave energy converter":['blue','wave & geothermal'],    
    
    "electricity production, natural gas, combined cycle power plant":['slategrey','gas'],
    "heat and power co-generation, natural gas, conventional power plant, 100MW electrical":['slategrey','gas'],
    "electricity production, oil":['black','oil and coal'],
    "electricity production, hard coal":['black','oil and coal'],
    
    
    "electricity production, hydro, pumped storage, Tr2050":['rebeccapurple','storage'],
    "electricity production, from hydrogen, with gas turbine, for grid-balancing, Tr2050":['rebeccapurple','storage'],
    "electricity production, from vehicle-to-grid, Tr2050":['rebeccapurple','storage'],
    "electricity supply, high voltage, from vanadium-redox flow battery system, Tr2050":['rebeccapurple','storage'],
...
```python
act_name_list=[
    "market for electricity, high voltage, Tr2050",
...
df_elec_2=df.style.background_gradient(cmap='Reds',subset='impact')
df_elec_2
```
...
for df in selected_db_list: 
    ...
    for diki in list_dict_storage:
        ...
        df.loc[len(df.index)] = [db.name,db.model, db.IAM_scenario,db.FR_scenario,db.year,db.warning,act_storage_name,exc_amount]
    #For each db in the selected list add the dataframe to the list of dataframes
...
for db in selected_db_list: 
    df=pd.DataFrame([],columns=['db_name','model','IAM scenario','FR scenario','year','warning','act','elec from storage (1kWh)','storage infrastructure','input elec losses','input elec 1kWh','unit','% storage infrastructure','% input elec losses','% input elec 1kWh','% efficency',"amount in elec market"])    
    ...
    for diki in list_dict_storage:
        ...
        #Store scores in a dataframe
        df.loc[len(df.index)] = [db.name,db.model, db.IAM_scenario,db.FR_scenario,db.year,db.warning,act_storage_name,total,infra,losses,score_elec,unit_impact,infra_contrib,losses_contrib,score_elec_contrib,efficency,exc_amount]
    #For each db in the selected list add the dataframe to the list of dataframes
...
impact_cat=climate
impact_cat=climate_premise
selected_db_list=[selected_db_list[2]] #premise_db_list
```
...
def plot_bar_graph_disagreg_contrib(list_df_to_plot, column, figsize=(10, 6)):
    ...
    plt.title('Origin of electricity')
...
```python editable=true slideshow={"slide_type": ""}
xlsx_file_name="export-test-03.xlsx"

list_df_to_export=[
    #["elec 1 kWh", df_elec_1, df_elec_2],
